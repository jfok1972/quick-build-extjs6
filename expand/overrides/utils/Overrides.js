...
Ext.apply(Ext.util.Format, {
  ...
  DictionaryRenderer : function(val, metaData, model, row, col, store, gridview) {
    var column = gridview.ownerCt.columnManager.getHeaderAtIndex(col);
    // model.clear()的时候，column的值会为人，奇怪了
    if (column) return html2Escape(model.get(column.dataIndex + '_dictname'));
  },
  // 金额字段
  ...
  iconClsRenderer : function(val) {
    if (val) return '<span class="' + html2Escape(val) + '"> ' + html2Escape(val) + '</span>';
    else return '';
  },
...
html2Escape = function(sHtml) {
	  if (!sHtml)
		  ...
		  return sHtml.replace(/[&]/g,'&amp;').replace(/[<]/g,'&lt;').replace(/[>]/g,'&gt;').replace(/["]/g,'&quot;');
}
