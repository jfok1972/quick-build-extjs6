Ext.define('app.view.MainController', {
  ...
  initCfg : function() {
    ...
    Ext.Ajax.on('requestexception', function(conn, response, options) {
      //spring mvc 返回的response.status为999，response.responseText为null
      //Spring boot 返回的status为500，response.responseText中的status为999,
      var status = response.status;
      if (status == '500' && !!response.responseText) { //为处理spring boot 的返回的出错信息
        var s = Ext.decode(response.responseText, true);
