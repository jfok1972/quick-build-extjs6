Ext.define('app.view.platform.module.toolbar.ToolbarController', {
          ...
          items : [{
                ...
                listeners : {
                  render : function(component) {
                    component.el.dom.src = cfg.pdfjsViewer + "?file=" + escape(component.src);
                  }
                },
  ...
  onExportTreeExeclButtonClick : function(button) {
    var me = this;
    me.exportExcel({
      formschemeid : button.formschemeid,
      formschemetitle : button.text
    });
  },
  exportExcel : function(parameters) {
    var me = this,
      ...
      params = {};
    if (parameters.formschemeid) {
      params.formschemeid = parameters.formschemeid;
      params.formschemetitle = parameters.formschemetitle;
    }
    if (parameters.toPdf) {
      params.topdf = true;
    }
    ...
    Ext.apply(params, {
      onlyselected : me.lookupReference('onlyselected').getValue(),
      ...
      unitalone : me.lookupReference('unitalone').getValue(),
      pagesize : me.lookupReference('pageA4').checked ? 'A4' : me.lookupReference('pageA4landscape').checked ? 'A4landscape' : me
        .lookupReference('pageA3').checked ? 'A3' : me.lookupReference('pageA3landscape').checked ? 'A3landscape' : 'pageautofit',
      autofitwidth : me.lookupReference('pageautofitwidth').checked,
      scale : me.lookupReference('scale').getValue()
    })
    Ext.apply(params, store.lastExtraParams);
    if (lastOptions.filters || params.onlyselected) {
      var filters = [];
      ...
      if (params.onlyselected) {
        //filter: [{'operator':'like','value':'\u672a','property':'title'}]
        filters.push({
          property : grid.moduleInfo.fDataobject.primarykey,
          operator : 'in',
          value : grid.getSelectionIds().join(',')
        })
      }
    ...
    if (parameters.thispage) {
      ...
      params.page = 1;
      params.start = 0;
      params.limit = 1000000  // xls 最大 65536行 ，xlsx 最大 1048576 , 65000
    }
    // 只导出选中的记录
    if (params.onlyselected) {
      // 文字说明
      var sl = grid.getSelectionModel().getSelection().length;
      if (sl == 0) {
        EU.toastInfo('没有选择导出的记录！');
        return;
      }
      conditions.push({
        property : '选中的',
    ...
    if (grid.moduleInfo.fDataobject.hassqlparam) {
      // 把所有选中的条件加进去
      var sqlparam = grid.moduleInfo.fDataobject.fDataobjectsqlparams;
      var sqlvalues = grid.getSqlParam();
      Ext.each(sqlparam, function(p) {
        if (sqlvalues[p.paramname]) conditions.push({
          property : p.title,
          operator : ':',
          value : sqlvalues[p.paramname]
        })
      })
    }
