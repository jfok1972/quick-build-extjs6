...
// 将buttons的参数加到这里,可以在定义的时候写在附加参数里面
  ...
  refreshPanel : function(model) {
    ...
    var optype = me.up('form').operatetype;	// 只有审核和显示状态下才能显示审核
    if ((optype != 'approve' && optype != 'display') || !me.model || !me.model.canApprove()) {
      me.hide();
  ...
  submitQuery : function(button) {
    var panel = button.up('approvepanel'),
      fields = panel.query('field'),
      hasError = false;
    Ext.each(fields, function(field) {
      hasError = hasError || !field.isValid();
    })
    if (hasError) {
      EU.toastWarn('表单检验未通过，请重新录入数据！');
      return;
    }
    Ext.MessageBox.confirm('确认' + button.text, '是否确定要执行『' + button.text + '』的操作?', function(btn) {
      if (btn == 'yes') {
        panel.submitResult(button);
      }
    });
  },
