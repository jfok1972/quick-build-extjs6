...
Ext.define('app.view.platform.module.ModuleInfo', {
  ...
  getNewPanelWithParentFilter : function(parentModuleName, fieldahead, pid, ptitle, param, operator) {
    // 如果parent只是一个filter,并不是一个模块
    var pf = {};
    if (parentModuleName == 'filter') {
      pf = {
        moduleName : this.fDataobject.objectname,
        fieldahead : null,
        fieldName : fieldahead,
        fieldtitle : ptitle,
  ...
  getPanelWithParentFilter : function(parentModuleName, fieldahead, pid, ptitle, param, operator) {
    // if (!this.canBrowseThisModule()) return null;
    if (!this.modulePanelWithParent) {
      this.modulePanelWithParent = this
        .getNewPanelWithParentFilter(parentModuleName, fieldahead, pid, ptitle, param, operator);
    } else {
      if (parentModuleName == 'filter') {
