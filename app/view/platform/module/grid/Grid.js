Ext.define('app.view.platform.module.grid.Grid', {
  ...
  listeners : {
    ...
    resize : function(panel) {
      // 如果有moduleview那么在改变尺寸之后，需要更新此view,否则左右可能对不齐
      if (panel.down('moduleview')) {
        panel.updateLayout();
      }
    }
  },
  initComponent : function() {
    ...
    if (me.moduleInfo.hasSummaryField()) {
      me.features = [{
            ftype : 'summary',
            remoteRoot: dataobj.hasremotesummary ? 'remoteRoot' : undefined,//后台提供总计数据的路径
            dock : 'bottom'
          }, {
    ...
    if (me.moduleInfo.fDataobject.rowbodytpl) {
      ...
      // 如果rowbodytpl的值为'loading...',那么就到后台去获取。
      if (me.moduleInfo.fDataobject.rowbodytpl == 'loading...') {
        Ext.apply(viewListeners, {
          expandbody : 'onAsyncExpandBody'
        })
      }
    }
    ...
    me.viewConfig.listeners = viewListeners;
    ...
    me.dockedItems = [moduletoolbar, dataobj.hassqlparam ? {
          xtype : 'modulesqlform',
          moduleInfo : me.moduleInfo,
          initSqlparam : me.modulePanel.sqlparam,
          dock : 'top'
        } : null, {
