...
Ext.define('app.view.platform.module.grid.GridStore', {
  ...
  listeners : {
    ...
    beforeload : function(store) {
      ...
      if (store.dataminingFilter) {
        store.proxy.extraParams.dataminingFilter = Ext.encode(store.dataminingFilter);
      } else delete store.proxy.extraParams.dataminingFilter;
    ...
    groupchange : function(store, grouper, eOpts) {
      if (grouper == null) {
        // 取消分组后,lastOptions并不改变
        delete store.lastOptions.grouper;
      }
    }
  },
