Ext.define('app.view.platform.module.grid.GridController', {
  ...
  onAsyncExpandBody : function(rowNode, record, expandRow, eOpts) {
    EU.RS({
      url : 'platform/dataobject/getexpandbody.do',
      params : {
        moduleName : this.getView().moduleInfo.fDataobject.objectname,
        recordId : record.getIdValue()
      },
      disableMask : true,
      callback : function(result) {
        if (result.success && expandRow.lastChild.firstChild) {
          var node = expandRow.lastChild.firstChild;
          if (result.msg) node.innerHTML = result.msg;
          else node.innerHTML = '暂无内容'
        }
      }
    })
  },
  ...
  afterGridRender : function(grid) {
    ...
    subitems.push({
      ...
      value : 'behindnumber',
      checked : false,
      handler : this.onMonetaryPositionChange,
      group : grid.getId() + '_position_group'
    });
    subitems.push({
      text : '显示在列头上',
      value : 'columntitle',
  ...
  onMonetaryPositionChange : function(menuitem) {
    menuitem.up('modulegrid').getViewModel().set('grid.monetaryPosition', menuitem.value);
  },
  columnMenuShow : function(menu) {
    ...
    if (m) {
      if (h.fieldDefine && h.fieldDefine.ismonetary) {
        var item = menu.down('menuitem[value=' + grid.getViewModel().get('grid.monetaryUnit') + ']');
        item.setChecked(true, true);
        item = menu.down('menuitem[value=' + grid.getViewModel().get('grid.monetaryPosition') + ']');
        item.setChecked(true, true);
        m.show();
        m.previousNode().show();
      } else {
  ...
  onGridRowDblClick : function(gridview, record, tr, rowIndex, e, eOpts) {
    ...
    if (grid.moduleInfo.fDataobject.rowbodytpl) return;
    if (rowedit) return;
