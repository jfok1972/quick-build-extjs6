...
Ext.define('app.view.platform.module.grid.ColumnsFactory', {
  ...
  requires : ['Ext.grid.column.Check', 'app.view.platform.module.grid.column.RecordIcon',
      ...
      'app.view.platform.module.grid.column.Aggregate','app.utils.CommonUtils'],
  statics : {
    /**
    ...
    getColumns : function(module, scheme, grid) {
      ...
      if (module.fDataobject.istreemodel) {
        if (result[0].xtype == 'manytoonefieldcolumn') result[0].dataIndex = result[0].manytooneNameName;
        result[0].xtype = 'treecolumn';
      }
    ...
    buildTextAndUnit : function() {
      var me = this,
        ...
        result = CU.fieldTitleTransform(me.gridField.title || me.fieldDefine.fieldtitle);
    ...
    getMenuText : function(fieldDefine, gridField) {
      ...
      return CU.fieldTitleTransform(result.replace(new RegExp('--', 'gm'), ''));
    },
        ...
        case 'percent' :
          Ext.apply(field, {
            ...
            widget : {
              ...
              animate : true,
              textTpl : ['<tpl if="value">{roundPercent}%</tpl>']
              //'{percent:number("0")}%' 或者 '{value*100}%'，这个有误差
            }
          })
          break;
        case 'blob' :
