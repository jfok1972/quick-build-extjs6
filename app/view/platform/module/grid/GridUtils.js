Ext.define('app.view.platform.module.grid.GridUtils', {
  ...
  setDataminingFilter : function(dataminingFilter,sqlparam) {
    var me = this;
    var sqlform = me.down('modulesqlform');
    if (sqlform)
    	sqlform.setValues(sqlparam ? sqlparam : {});
  ...
  updateTitle : function() {
    ...
    // 如果有父模块的filter，则加进来
    if (me.parentFilter) {
      if (me.parentFilter.fieldtitle === me.parentFilter.text) {
  ...
  _getExportGridColumns : function(items) {
    ...
    Ext.each(items, function(item) {
      ...
      if (item.items.length === 0 || (item.dataIndex && item.isExport)) {
