...
Ext.define('app.view.platform.module.Module', {
  ...
  setSqlparam : function(sqlparam){
	    var me = this;
	    if (Ext.encode(me.sqlparam) == Ext.encode(sqlparam)) return;
	    me.sqlparam = sqlparam;
	    // me.getModuleGrid().setSqlparam(sqlparam);
  },
  setDataminingFilter : function(dataminingFilter , sqlparam) {
    var me = this;
    if (!sqlparam)
    	sqlparam = null;
    if (Ext.encode(me.dataminingFilter) == Ext.encode(dataminingFilter)
    		&& Ext.encode(me.sqlparam) == Ext.encode(sqlparam)) return;
    me.dataminingFilter = dataminingFilter;
    me.sqlparam = sqlparam;
    me.getModuleGrid().setDataminingFilter(dataminingFilter, sqlparam);
  },
