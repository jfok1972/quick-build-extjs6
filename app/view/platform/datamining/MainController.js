Ext.define('app.view.platform.datamining.MainController', {
  ...
  init : function() {
    ...
    var sqlform = me.lookupReference('modulesqlform');
    if (sqlform){
    	viewmodel.set('sqlparam' , sqlform.getSqlParam());
    }
  ...
  onDataDetailChange : function(condition) {
    var me = this,
      ...
      sqlparam = viewmodel.get('sqlparam');
    if (viewmodel.get('datadetail.inWindow') == 'true') {
      ...
      me.detailWindow.down('modulepanel').setDataminingFilter(condition , sqlparam);
      ...
      if (!detail) {
        ...
        detail.down('modulepanel').setDataminingFilter(condition, sqlparam);
        if (detail.collapsed) {
          detail.expand();
        }
      }
  ...
  exportExcelOrPdf : function(topdf) {
    var me = this,
      ...
      fDataobject = vm.get('moduleInfo').fDataobject,
    ...
    params.moduletitle = fDataobject.title;
    params.schemename = vm.get('currentScheme').text ? vm.get('currentScheme').title : '未保存的数据分析方案';
    var conditions = [];
    if (fDataobject.hassqlparam) {
        // 把所有选中的条件加进去
        var sqlparam = fDataobject.fDataobjectsqlparams;
        var sqlvalues = vm.get('sqlparam');
        Ext.each(sqlparam, function(p) {
          if (sqlvalues[p.paramname]) 
        	conditions.push({
              source : '查询参数',
              fieldtitle : p.title,
              operator : ':',
              displaycond : sqlvalues[p.paramname]
          })
        })
      }
    var condarray = condition.getExportString();
    for (var i in condarray)
    	conditions.push(condarray[i]);
    ...
    params.scale = me.lookupReference('scale').getValue();
    // 所有当前显示的列
  ...
  onSqlparamChanged: function(sqlparam) {
	  var me = this;
	  me.getViewModel().set('sqlparam', sqlparam);
