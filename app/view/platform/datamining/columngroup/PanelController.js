Ext.define('app.view.platform.datamining.columngroup.PanelController', {
  ...
  requires : ['app.view.platform.datamining.columngroup.editColumnWindow'],
  // 选中一条记录，然后复制一个，用于新增一个字段
  addColumn : function(button) {
    var me = this,
      columntree = me.lookupReference('selectedcolumntree'),
      sm = columntree.getSelectionModel();
    if (sm.getSelection().length != 1) {
      EU.toastInfo('请选择一条需要复制新增的记录！');
      return;
    }
    var first = sm.getSelection()[0];
    var rec = {
      text : first.get('text') + '(复制)',
      condition : first.get('condition'),
      value : first.get('value'),
      leaf : true
    }
    var inserted = first.parentNode.insertChild(first.parentNode.indexOf(first) + 1, rec);
  },
  // 双击后显示condition text value属性
  onTreeItemDblClick : function(tree, record, item, index, e, eOpts) {
    var window = Ext.widget('dataminingcolumngroupeditcolumnwindow', {
      record : record
    });
    window.show();
  },
  ...
  genFieldColumns : function(fieldid, title, parentnode) {
    var me = this,
      ...
      params = {
        ...
        sqlparamstr : Ext.encode(me.getViewModel().get('sqlparam')),
      };
