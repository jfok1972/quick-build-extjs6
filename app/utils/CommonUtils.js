...
Ext.define('app.utils.CommonUtils', {
  ...
  statics : {
	// 将某些相对字段，改成绝对字段如：下01个月， 前01个月，改成 20年08月 ，20年06月。
	transformArray : [],
	fieldTitleTransform : function(title) {
		var now = new Date();
		if (this.transformArray.length == 0) {
			for ( var i = 1 ; i<=12 ;i ++){
				this.transformArray.push({
					sour : '前' + (i<10 ? '0' :'') + i +'个月',
					tranto : Ext.Date.format(Ext.Date.add(now,'mo',-i),"y年m月")
				}) 
				this.transformArray.push({
					sour : '下' + (i<10 ? '0' :'') + i +'个月',
					tranto : Ext.Date.format(Ext.Date.add(now,'mo',i),"y年m月")
		...
		if (title){
			for (var i=0;i<this.transformArray.length;i++){
				title = title.replace(this.transformArray[i].sour , this.transformArray[i].tranto);
			}
		}
		return title;
	},
	 ...
	 *            propname
	 * @param {}
	 *            value //name,'张三', 返回 {name : '张三'} prop.name,'张三', 返回 { prop :
	 ...
	 * 
	 * @param {}
	 *            value 2015=2015-01-01、2015-02 = 2015-02-01
	 * @return {}
	 */
    ...
    /**
	 * 将字符串转换成竖形排列的，如 网站首页，改成 网<br/>站<br/>首<br/>页, 如果有iconCls,则改成<span
	 * class="x-fa fa-home"/><br/>网<br/>站<br/>首<br/>页
	 * 
	 * @param {}
	 *            text
	 * @param {}
